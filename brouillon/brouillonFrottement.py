...
def main():
    ...
    for ti in range(lenT):
        ...
        def systemeDebitUniformeLoiFrottement(listOfArgs, n):
            # Reconstruction des vecteurs
            J = np.array(listOfArgs[:n-1])
            h = np.array(listOfArgs[n-1:2*n])
            Qs = np.array(listOfArgs[2*n:3*n])
            z = np.array(listOfArgs[3*n:4*n])
            A = b*h
            P = b+2*h
            Rh = A/P
            listOfEquations = []
            ...
            # Loi de frottement
            listOfEquations += list(Ks*A*R_h**(2/3)*J**0.5-Q[ti])
            ...
            for k in range(len(J)):
                ...
                listOfEquations += Qs-np.minimum(fndict[loi](*argsdict[loi]),QsMin)

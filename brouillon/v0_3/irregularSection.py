...
from utils import G, Y_MIN, get_centroid

class IrregularSection:

    def __init__(self, points, x, z, z_min=None, up_section=None, down_section=None, granulometry=None, manning=None):
        """
    ...
    def interp_as_down_section(self, otherSection, x=None):
        interpolatedSection = IrregularSection.interp(otherSection, self, x=x)
        if x==None:
            x = 0.5*(self.get_x()+otherSection.get_x())
        return interpolatedSection

    def copy(self):
        """return a safe copy of this section"""
        return IrregularSection(self.__points[:], self.get_x(), self.get_z(), z_min=self.get_z_min(), up_section=self.get_up_section(), down_section=self.get_down_section(), granulometry=self.get_granulometry(), manning=self.get_manning())

    def get_stored_vol(self):  
    ...
    def get_wet_section(self, y):
        ...
        if y >= self.get_y_max():
            print(f"WARNING : Water depth has gone upper than the maximum one ({self.__y_max}m)")
            return points        
    ...
    def get_Sf(self, Q, y, wet_points=None):
        ...
        wet_points = self.get_wet_section(y) if wet_points==None else wet_points
        return  (self.get_Cf(Q, y, wet_points) * self.get_V(Q, y, wet_points)**2) / (G*self.get_R(y, wet_points)) #(self.get_manning()*self.get_V(Q, y, wet_points)/(self.get_R(y, wet_points)**(2/3)))**2
    
    def get_Cf(self, Q, y, wet_points=None):
    ...
    def get_yc(self, Q):
        ...
        def equation_function(y): # Froude number is one for critical depth
            ...
            return 1-Fr
        try:
            yc = op.brentq(equation_function, Y_MIN, 0.999*self.get_y_max())
        except ValueError:  # f(a) and f(b) must have different signs
            try:
                yc = op.newton(equation_function, Y_MIN, tol=0.001, maxiter=1000) # Utilisation de la méthode de la sécante
    ...
    def get_yn(self, Q):
        ...
        def equation_function(y): # Friction law for a uniform regim leads to this equation
            wet_points = self.get_wet_section(y)
            sf = self.get_Sf(Q, y, wet_points=wet_points)
            s0 = self.get_S0(up_direction=b)
            return sf-s0
        try:
            yn = op.brentq(equation_function, Y_MIN, 0.999*self.get_y_max())
        except ValueError:  # f(a) and f(b) must have different signs
            try:
                yn = op.newton(equation_function, Y_MIN, tol=0.001, maxiter=1000)
                print("Warning : brentq method failed to converge for yn, used Newton")
            except RuntimeError as e:  # Failed to converge
                print(f"WARNING : Newton method failed to converge.")
                raise(e)
    ...
    def interp(up_section, down_section, x=None):
        ...
        Returns :
            interpolatedSection (Irregular section) : section with interpolated attributes. 
        ...
        interpolated_section.__s0 = (up_section.__z - down_section.__z) / (down_section.__x - up_section.__x)
        interpolated_section.__manning = np.interp(x, [up_section.__x, down_section.__x], [up_section.__manning, down_section.__manning])
        interpolated_section.__points = copy(up_section.__points)
        interpolated_section.x_list = copy(up_section.__x_list)
        interpolated_section.y_list = copy(up_section.__x_list)
